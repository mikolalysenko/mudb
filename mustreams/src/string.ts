class TextEncoder {
    public encode (str:string) {
        return new Uint8Array(this._utf8ToBytes(str));
    }

    // Taken from https://github.com/feross/buffer
    private _utf8ToBytes (str:string) : number[] {
        let codePoint;
...
class TextDecoder {
    ...
    public decode (bytes:Uint8Array) : string {
        ...
        return this._decodeCodePointsArray(res);
    }

    private _decodeCodePointsArray (codePoints:number[]) : string {
